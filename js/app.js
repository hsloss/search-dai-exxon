...
let Variables = function(name, region, description){
...
let clickHandler = function(){
  ...
  for(let i = 0; i < VariablesArray.length; i++){
    if(input.value === VariablesArray[i].countryName){
      selectedCountry.push(VariablesArray[i])
      console.log(VariablesArray[i])
      let definition = document.createElement('p')
      paragraph.appendChild(definition)
      definition.innerText = VariablesArray[i].countryDescription
    }
  }
  removeSeeAlso()
  for(let i = 0; i < VariablesArray.length; i++){
    let j=0
    if (VariablesArray[i].countryRegion === selectedCountry[j].countryRegion|| VariablesArray[i].countryLanguage === selectedCountry[j].countryLanguage) {
      let elRow = document.createElement('li')
      elTable.appendChild(elRow)
      elRow.innerText = VariablesArray[i].countryName
    }
  }
  selectedCountry = []
}
...
let populateForm = function() {
  ...
  for (let i = 0; i < VariablesArray.length; i++) {
    ...
    option.innerText = VariablesArray[i].countryName
  }
}
